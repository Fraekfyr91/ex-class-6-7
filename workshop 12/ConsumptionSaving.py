...
def plotting(ax, x,y, title, xlabel, ylabel, ylim, xlim):
    '''
    Helping function for displaying the model outcome
    args:
        ax (matplotlib axis): the axis to plot on
        x (list): list of numpy arrays to display the x_axis
        y (list): list of numpy arrays to display the value or consumption
        title (string): title to the figure
        xlabel (string): sting that specifies the x-axis
        ylabel (string): string that specifies the y-axis
        ylim (list): list that have min-y and max-y
        xlim (list): list that have min-x and max-x

    '''
    #plot the values
    ax.plot(x[0],y[0], label=f'Period {1}')
    ax.plot(x[1],y[1], label=f'Period {2}')
    # create legend
